...
function normalize(v){return clean(v).replace(/\s+/g," ").toLowerCase();}
function ensureRound(team,key){
    team.progress=team.progress||{};
    team.progress[key]=team.progress[key]||{score:0,hintsUsed:0,wrongSubmissions:0,startedAt:null,completedAt:null,stages:{}};
    return team.progress[key];
}
function ensureStage(team,key,index){
    const round=ensureRound(team,key), sk=`stage${index}`;
    round.stages[sk]=round.stages[sk]||{startedAt:null,completedAt:null,score:0,attempts:0,hintsUsed:[],lastAnswer:"",completionMs:null};
    return round.stages[sk];
}
function roundScore(team,key){return Number(team.progress?.[key]?.score||0);}
function qualificationScore(team){return ORDER.reduce((s,k)=>s+roundScore(team,k),0);}
function phaseFrom(control, now=Date.now()){
    if(!control?.startedAt) return {key:"waiting_start",endsAt:null};
    const elapsed=Math.max(0,Math.floor((now-new Date(control.startedAt).getTime())/1000));
    if(elapsed<STARTS.round2) return {key:"round1",endsAt:new Date(new Date(control.startedAt).getTime()+STARTS.round2*1000)};
    if(elapsed<STARTS.round3) return {key:"round2",endsAt:new Date(new Date(control.startedAt).getTime()+STARTS.round3*1000)};
    if(elapsed<STARTS.surprise) return {key:"round3",endsAt:new Date(new Date(control.startedAt).getTime()+STARTS.surprise*1000)};
    if(elapsed<STARTS.final) return {key:"surprise",endsAt:new Date(new Date(control.startedAt).getTime()+STARTS.final*1000)};
    if(elapsed<FINAL_END) return {key:"final",endsAt:new Date(new Date(control.startedAt).getTime()+FINAL_END*1000)};
    return {key:"completed",endsAt:new Date(new Date(control.startedAt).getTime()+FINAL_END*1000)};
}
...
function totalWrong(team,key=null){return key?Number(team.progress?.[key]?.wrongSubmissions||0):Number(team.wrongSubmissions||0);}
function totalCompletion(team,key){
    const stages=Object.values(team.progress?.[key]?.stages||{});
    return stages.reduce((sum,s)=>sum+(Number.isFinite(s?.completionMs)?s.completionMs:0),0)||Number.MAX_SAFE_INTEGER;
}
async function finalizeQualification(){
    ...
    teams.sort((a,b)=>{
        ...
        const sa=roundScore(a,"surprise"),sb=roundScore(b,"surprise"); if(sa!==sb)return sb-sa;
        const r3a=roundScore(a,"round3"),r3b=roundScore(b,"round3"); if(r3a!==r3b)return r3b-r3a;
        if(a.wrongSubmissions!==b.wrongSubmissions)return a.wrongSubmissions-b.wrongSubmissions;
        return totalCompletion(a,"surprise")-totalCompletion(b,"surprise");
    });
    for(let i=0;i<teams.length;i++){
        teams[i].rank=i+1;
        teams[i].currentRound=i<3?"final":"eliminated";
        teams[i].currentStage=1;
        await teams[i].save();
    }
}
async function finalizeFinal(){
    ...
    finalists.sort((a,b)=>{
        const fa=roundScore(a,"final"),fb=roundScore(b,"final"); if(fa!==fb)return fb-fa;
        const ha=Number(a.progress?.final?.hintsUsed||0),hb=Number(b.progress?.final?.hintsUsed||0); if(ha!==hb)return ha-hb;
        const wa=totalWrong(a,"final"),wb=totalWrong(b,"final"); if(wa!==wb)return wa-wb;
        const ta=totalCompletion(a,"final"),tb=totalCompletion(b,"final"); if(ta!==tb)return ta-tb;
...
async function syncTeam(team){
    ...
    if(phase.key==="waiting_start") {if(team.currentRound!=="eliminated"&&team.currentRound!=="completed")team.currentRound="waiting_start";return {control,phase};}
    if(ORDER.includes(phase.key)){
        ...
        if(!team.rank) await finalizeQualification();
        const fresh = await BugHuntTeam.findById(team._id);
        if (fresh) { team.rank=fresh.rank; team.currentRound=fresh.currentRound; team.currentStage=fresh.currentStage; team.qualificationScore=fresh.qualificationScore; team.finalPlace=fresh.finalPlace; }
