...
function createTeamToken(team){const now=Math.floor(Date.now()/1000),payload=Buffer.from(JSON.stringify({teamId:team.teamId,event:"Bug Hunt",role:"bughunt-team",iat:now,exp:now+TOKEN_LIFETIME_SECONDS})).toString("base64url");return `${payload}.${sign(payload)}`}
function verifyTeamToken(token){try{const [payload,signature,extra]=String(token||"").split(".");if(!payload||!signature||extra||!safeEqual(signature,sign(payload)))return null;const data=JSON.parse(Buffer.from(payload,"base64url").toString("utf8"));if(data.role!=="bughunt-team"||!data.teamId||!data.exp||data.exp<=Math.floor(Date.now()/1000))return null;return data}catch{return null}}
